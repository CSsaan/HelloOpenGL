...
void CSOpenGLWedgetSharpen::checkGLRrror(const char *msg)
{
    int err = glGetError();
    if (err != GL_NO_ERROR)
        printf("err=%d(%s)\n", err, msg);
...
// ############################################################################################
void CSOpenGLWedgetSharpen::myGetLocation()
{
...
void CSOpenGLWedgetSharpen::myUniformData()
{
...
CSOpenGLWedgetSharpen::CSOpenGLWedgetSharpen(QWidget *parent) : QOpenGLWidget(parent)
{
...
CSOpenGLWedgetSharpen::~CSOpenGLWedgetSharpen()
{
    cleanup();
}

void CSOpenGLWedgetSharpen::cleanup()
{
...
void CSOpenGLWedgetSharpen::initializeGL()
{
    qDebug() << "[sharpen]###########################################初始化###########################################";
    initializeOpenGLFunctions();
    glClearColor(0, 0, 0, 1);
    program = new QOpenGLShaderProgram;
    qDebug() << "[sharpen]Load Vert FIle：" <<program->addShaderFromSourceFile(QOpenGLShader::Vertex, QDir::currentPath()+ QString("/Shaders/SHARPEN.vert"));
    qDebug() << "[sharpen]Load Frag FIle：" <<program->addShaderFromSourceFile(QOpenGLShader::Fragment, QDir::currentPath()+ QString("/Shaders/SHARPEN.frag"));
...
void CSOpenGLWedgetSharpen::resizeGL(int w, int h)
{
...
void CSOpenGLWedgetSharpen::paintGL()
{
    time_begin();
...
//###############################################################################################
void CSOpenGLWedgetSharpen::time_begin()
{
    gettimeofday(&G_tv_begin,NULL);
}

void CSOpenGLWedgetSharpen::time_end()
{
...
void CSOpenGLWedgetSharpen::set_sharpenStr(int s)
{
    sharpenStr = s / 100.0f;
}

void CSOpenGLWedgetSharpen::savepicture()
{
    makeCurrent();
